...
class ApiRouter:
    ...
    async def booking_url(self, club_id: str, path: str) -> str:
        club_key = club_id.replace("-", "_").upper()
        override = os.environ.get(
            f"TWOJTENIS_BOOKING_API_URL_{club_key}"
        ) or os.environ.get("TWOJTENIS_BOOKING_API_URL")
        if override:
            return f"{override.rstrip('/')}{path}"
        tech = await self._resolver.service_url_for_club(club_id)
        return f"{tech}{path}"

    async def booking_get(
        self, club_id: str, path: str, *, client: ApiClient, params=None
    ) -> Any:
        """GET from the booking API. On 404, invalidates tech-group cache and retries once."""
        club_key = club_id.replace("-", "_").upper()
        url = await self.booking_url(club_id, path)
        try:
            return await client.get(url, access_token=None, params=params)
        except ApiErrorException as exc:
            if exc.code == "HTTP_ERROR" and (exc.details or {}).get("status") == 404:
                self._resolver.invalidate(club_id)
                url2 = await self.booking_url(club_id, path)
                if url2 == url:
                    override_key = f"TWOJTENIS_BOOKING_API_URL_{club_key}"
                    override_active = (
                        override_key in os.environ
                        or "TWOJTENIS_BOOKING_API_URL" in os.environ
                    )
                    if override_active:
                    ...
                    raise ApiErrorException(
                        "BOOKING_URL_MISMATCH", msg, exc.details
                    ) from exc
                return await client.get(url2, access_token=None, params=params)
            raise
